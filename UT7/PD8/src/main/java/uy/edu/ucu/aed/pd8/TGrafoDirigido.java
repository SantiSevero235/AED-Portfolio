package uy.edu.ucu.aed.pd8;

import java.util.*;

import static java.lang.Math.min;
import static uy.edu.ucu.aed.pd8.UtilGrafos.obtenerMatrizCostos;
...
public class TGrafoDirigido implements IGrafoDirigido {
    ...
    public boolean contieneCiclo() {
        ...
        Set<Comparable> enProceso = new HashSet<>();

        for (IVertice vertice : vertices.values()) {
            if (!visitados.contains(vertice.getEtiqueta())) {
                if (contieneCicloDFS(vertice, visitados, enProceso)) {
                    System.out.println("Ciclo detectado en el grafo.");
                    return true;
                }
            }
        }
    ...
    private boolean contieneCicloDFS(IVertice vertice, Set<Comparable> visitados, Set<Comparable> enProceso) {
        enProceso.add(vertice.getEtiqueta());
        System.out.println("Visitando vértice: " + vertice.getEtiqueta());

        for (IAdyacencia adyacencia : vertice.getAdyacentes()) {
            IVertice adyacente = adyacencia.getDestino();

            if (enProceso.contains(adyacente.getEtiqueta())) {
                System.out.println("Ciclo encontrado al volver a visitar el vértice: " + adyacente.getEtiqueta());
                return true;
            }
            if (!visitados.contains(adyacente.getEtiqueta())) {
                if (contieneCicloDFS(adyacente, visitados, enProceso)) {
                    return true;
                }
            }
        }

        enProceso.remove(vertice.getEtiqueta());
        visitados.add(vertice.getEtiqueta());
        return false;
    }

    public LinkedList<IVertice> clasificacionTopologica() {
        LinkedList<IVertice> listaOrdenada = new LinkedList<>();
    ...
    public boolean esFuertementeConexo() {
        for (IVertice vertice : getVertices().values()) {
            desvisitarVertices();
            if (!dfsDesdeVertice((TVertice) vertice)) {
                return false;
            }
            if (!verificarVisitas()) {
                return false;
    ...
    private boolean dfsDesdeVertice(TVertice vertice) {
        vertice.setVisitado(true);
        LinkedList<TAdyacencia> v = vertice.getAdyacentes();
        for (TAdyacencia adyacencia : v) {
            TVertice adyacente = (TVertice) adyacencia.getDestino();
            if (!adyacente.getVisitado()) {
                dfsDesdeVertice(adyacente);
            }
        }
        return true;
    }

    private boolean verificarVisitas() {
        for (IVertice vertice : getVertices().values()) {
            if (!vertice.getVisitado()) {
                return false;
            }
    ...
    public void clasificarArcos(List<TArista> arcosArbol,
        ...
        for (IVertice v : vertices.values()) {
            v.setVisitado(false);
            if (v instanceof TVertice) {
                ((TVertice<?>) v).setTiempoDescubrimiento(0);
                ((TVertice<?>) v).setTiempoFinalizacion(0);
            }
        }
        ...
        for (IVertice v : vertices.values()) {
            if (!v.getVisitado() && v instanceof TVertice) {
                ((TVertice<?>) v).clasificarArcos(arcosArbol, arcosRetroceso, arcosAvance, arcosCruzados, tiempoGlobal);
            }
        }
    }
}
