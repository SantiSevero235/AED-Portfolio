...
public class TVertice<T> implements IVertice {
    ...
    public LinkedList<IAdyacencia> getAdyacentes() {
        LinkedList<IAdyacencia> lista = new LinkedList<>();
    ...
    public void clasificacionTopologica(LinkedList<IVertice> listaOrdenada) {
        this.setVisitado(true);

        for (IAdyacencia adyacencia : adyacentes) {
            IVertice verticeDestino = adyacencia.getDestino();
    ...
    public LinkedList<TAdyacencia> getAdyacencias() {
        LinkedList<TAdyacencia> lista = new LinkedList<>();
        for (IAdyacencia ady : adyacentes) {
            lista.add((TAdyacencia) ady);
        }
        return lista;
    }

    public void setTiempoDescubrimiento(int t) {
        this.tiempoDescubrimiento = t;
    }

    public void setTiempoFinalizacion(int t) {
        this.tiempoFinalizacion = t;
    }
    ...
    public void clasificarArcos(List<TArista> arcosArbol,
                                ...
                                int[] tiempoGlobal) {
        this.setVisitado(true);
        this.setTiempoDescubrimiento(++tiempoGlobal[0]);

        for (IAdyacencia ady : adyacentes) {
            TVertice destino = (TVertice) ady.getDestino();

            if (!destino.getVisitado()) {
                arcosArbol.add(new TArista(this.etiqueta, destino.etiqueta, ady.getCosto()));
                destino.clasificarArcos(arcosArbol, arcosRetroceso, arcosAvance, arcosCruzados, tiempoGlobal);
            } else {
                if (destino.getTiempoFinalizacion() == 0) {
                    arcosRetroceso.add(new TArista(this.etiqueta, destino.etiqueta, ady.getCosto()));
                } else if (this.tiempoDescubrimiento < destino.getTiempoDescubrimiento()) {
                    arcosAvance.add(new TArista(this.etiqueta, destino.etiqueta, ady.getCosto()));
                } else {
                    arcosCruzados.add(new TArista(this.etiqueta, destino.etiqueta, ady.getCosto()));
                }
            }
        }

        this.setTiempoFinalizacion(++tiempoGlobal[0]);
    }
}
